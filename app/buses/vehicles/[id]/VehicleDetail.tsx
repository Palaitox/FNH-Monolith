...
export default function VehicleDetail({ vehicle, compliance, requirements, role }: Props) {
  ...
  function handleSaveDocs() {
    ...
    startTransition(async () => {
      try {
        await recordVehicleDocumentsAction(vehicle.id, documents)
  ...
  function handleDeactivate() {
    ...
    startTransition(async () => {
      await deactivateVehicleAction(vehicle.id)
    ...
    <div className="px-4 py-6 sm:px-6 max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-xl font-semibold tracking-tight font-mono">{vehicle.plate}</h1>
